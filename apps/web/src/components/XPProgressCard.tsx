...
const LEVEL_COLORS: Record<UnifiedProgressionLevel, { bg: string; text: string; border: string }> = {
...
export default function XPProgressCard({
  ...
  const levelThresholds = UNIFIED_XP_THRESHOLDS[currentLevel];
  ...
  // Next level info
  const levels: UnifiedProgressionLevel[] = ['BEGINNER', 'INTERMEDIATE', 'ADVANCED', 'ELITE'];
  const currentLevelIndex = levels.indexOf(currentLevel);
  const nextLevel = currentLevelIndex < levels.length - 1 ? levels[currentLevelIndex + 1] : null;
  const xpToNextLevel = nextLevel ? UNIFIED_XP_THRESHOLDS[nextLevel].min - currentXP : 0;

  // Use axis metadata for label if not provided
  const displayLabel = axisLabel || UNIFIED_AXIS_METADATA[axis].displayName;
